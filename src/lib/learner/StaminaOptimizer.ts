...
export class StaminaOptimizer {
    ...
    processReviewEvent(quality: number, isPseudoKnowledge: boolean) {
        ...
        if (quality < 3) {
            ...
            // Circadian Rhythm Factor: Late night (23:00 - 04:00) drains 2x more stamina
            const hour = new Date().getHours();
            const circadianFactor = (hour >= 23 || hour < 4) ? 2.0 : 1.0;

            this.msi -= (this.MSI_DROP_PER_CARD + (isPseudoKnowledge ? 1.0 : 0)) * circadianFactor;
        }
