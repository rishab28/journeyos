            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                {subjects.map((s) => (
                    <button
                        key={s}
                        onClick={() => {
                            setFilters({ subject: s as any, topic: null, chapter: null });
                            if (s !== 'Mixed') setIsExpanded(true);
                            else setIsExpanded(false);
                        }}
                        className={`px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider transition-all whitespace-nowrap border ${activeSubject === s
