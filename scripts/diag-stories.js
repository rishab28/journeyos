...
async function diag() {
  ...
  const { data: total, count } = await supabase.from('daily_stories').select('id', { count: 'exact', head: true });
  console.log('Total stories in daily_stories:', count);
  ...
  console.log('\nActive Stories (> now):', active?.length || 0);
